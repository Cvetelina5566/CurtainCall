...
User = get_user_model()  # Това гарантира, че Django взема правилния Custom User модел
...
def register_user(request):
    try:
        ...
        # Проверка за съществуващий потребител
    ...
    try:
        email = request.data.get("email")
        password = request.data.get("password") or request.data.get("password1") # Увери се, че от Next.js пращаш "password", а не "password1"
        ...
        # Ако нещо се счупи, това ще ни върне точната грешка като съобщение в браузъра
        return Response({"error": f"Вътрешна грешка: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
